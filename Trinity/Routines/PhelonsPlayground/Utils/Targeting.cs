    ...
    public static class Targeting
    {
        ...
        internal static List<TrinityActor> SafeList(bool objectsInAoe = true)
        {
            return
                Targets.Where(
                    x =>
                        !x.IsPlayer && !x.IsShadowClone && !x.IsIllusion && !x.IsSummonedByPlayer &&
                        (!x.IsUnit || x.IsUnit && x.HitPoints > 0) && //x.IsInLineOfSight &&
                        (objectsInAoe || !Core.Avoidance.InAvoidance(x.Position)) &&
                        !Core.Avoidance.InCriticalAvoidance(x.Position)).ToList();
        }
        ...
        {
            return
                Targets.Where(x => !x.IsPlayer && x.IsUnit && x.IsDead && x.IsInLineOfSight && x.Position.Distance2D(Player.Position) < 60f &&
        ...
        internal static TrinityActor Monk
        {
            get
            {
                try
                {
                    var monk = Actors.FirstOrDefault(x => x.IsPlayer && x.InternalName.ToLower().Contains("monk"));
                    //if (monk == null)
        ...
        internal static TrinityActor PullCharacter
        {
            ...
            {
                return
                    SafeList().OrderBy(y => y.NearbyUnitsWithinDistance(20))
                ...
                UnitSafeList(includeUnitsInAoe)
                    .Where(u => u.IsUnit && u.CanCastTo() && u.IsInLineOfSight &&
                                !UnitsAroundPuller(pullLocation, groupRadius, includeUnitsInAoe)
                                    .Select(x => x.AcdId)
                                    .Contains(u.AcdId) && 
                                !UnitsAroundPlayer(groupRadius, includeUnitsInAoe)
                                    .Select(x => x.AcdId)
                                    .Contains(u.AcdId) && u.Position.Distance(pullLocation) <= searchRange)
                    .OrderByDescending(u => u.Distance).ToList();
        }
        ...
        internal static bool BestBuffPosition(float maxRange, Vector3 fromLocation, bool objectsInAoe, out Vector3 location)
        {
            ...
            if (ZetaDia.Me.IsParticipatingInTieredLootRun || Core.Player.IsInventoryLockedForGreaterRift)
            {
                ...
                if ((closestOcc != Vector3.Zero || AnyElitesChampionsBossesInRange(45f) ||
                     ...
                     CurrentTarget != null &&
                     (CurrentTarget.MonsterAffixes.HasFlag(MonsterAffixes.Frozen) ||
                      CurrentTarget.MonsterAffixes.HasFlag(MonsterAffixes.ArcaneEnchanted))) &&
        ...
        internal static List<TrinityActor> BestShrine(float range = 25f, bool objectsInAoe = true)
        {
            return
            (from u in SafeList(objectsInAoe)
                where u.Position.Distance2D(Player.Position) <= range &&
                      u.Type == TrinityObjectType.Shrine
                orderby u.Distance
                select u).ToList();
        }

        internal static Vector3 BestDpsPosition(Vector3 location, float searchRange = 12f, bool objectsInAoe = true)
        {
        ...
        internal static Vector3 BestTankPosition(float maxRange, bool objectsInAoe = true)
        {
        ...
        internal static Vector3 BestWalkLocation(float maxRange, bool objectsInAoe = true)
        {
            ...
            int count = 1, bool useWeights = true, bool includeUnitsInAoe = true)
        {
            return UnitSafeList(includeUnitsInAoe)
                ...
                .FirstOrDefault(u => ((useWeights && u.Weight > 0) || !useWeights) &&
                                     ...
                                     u.Position.Distance2D(Player.Position) <= maxRange &&
        ...
        {
            if (position.Distance2D(Player.Position) > 30f)
                position = MathEx.CalculatePointFrom(Player.Position, position, 30f);

            return Grids.Avoidance.IsIntersectedByFlags(Player.Position, position, AvoidanceFlags.Monster);
        }
            ...
            bool ignoreElites = false, bool inLineOfSight = false)
        {
            return UnitSafeList(includeUnitsInAoe).Where(u => u.IsUnit &&
                ...
                .ThenByDescending(u => u.HitPointsPct);
        }
        ...
        internal static List<TrinityActor> GetOculusBuffDiaObjects(float range, Vector3 fromLocation, bool objectsInAoe = true)
        {
            //[1FABA194] Type: ClientEffect Name: p2_itemPassive_unique_ring_017_dome-58267 ActorSnoId: 433966, Distance: 24.701

            return SafeList(objectsInAoe).OrderBy(x => NearbyUnitsWithinDistance(x, 15f)).Where(u => fromLocation.Distance2D(u.Position) <= range && Core.Avoidance.Grid.CanRayCast(fromLocation, u.Position) && 
                                               u.ActorSnoId == 433966).OrderBy(u => u.Distance).ToList();
        }

        internal static Vector3 ClosestSancAndOcc(float maxRange, Vector3 fromLocation, bool objectsInAoe = true)
        {
        ...
        internal static Vector3 ClosestSanctuary(float maxRange, Vector3 fromLocation, bool objectsInAoe = true)
        {
            var TrinityActor = GetInnerSanctuaryDiaObjects(maxRange, fromLocation, objectsInAoe).OrderBy(x => x.IsSafeSpot).FirstOrDefault();
            return TrinityActor?.Position ?? Vector3.Zero;
        }

        internal static List<TrinityActor> GetInnerSanctuaryDiaObjects(float range, Vector3 fromLocation,
            bool objectsInAoe = true)
        {
            return SafeList(objectsInAoe).Where(u =>
                fromLocation.Distance2D(u.Position) <= range &&
                Core.Avoidance.Grid.CanRayCast(fromLocation, u.Position) &&
                u.ActorSnoId == 320136).OrderBy(u => u.Distance).ToList();
        }

        internal static TrinityActor ClosestGlobe(float distance = 45, bool objectsInAoe = true)
        {
            return SafeList(objectsInAoe).OrderBy(x => x.Distance).FirstOrDefault(u => (u.Type == TrinityObjectType.HealthGlobe || u.Type == TrinityObjectType.PowerGlobe) &&
                            u.Position.Distance2D(Player.Position) <= distance);
        }
        ...
        internal static Vector3 GetDiaObjectBestClusterPoint(uint actorSNO, float radius = 15f, float maxRange = 45f,
            bool useWeights = true, bool includeUnitsInAoe = true, bool objectsInAoe = true)
        {
            var clusterUnits =
            (from u in SafeList(objectsInAoe)
                ...
                u.Position.Distance2D(Player.Position) descending
                select u.Position).ToList();

            return clusterUnits.Any() ? clusterUnits.FirstOrDefault() : Vector3.Zero;
        }

        internal static List<TrinityActor> GetTwisterDiaObjects(float range = 25f, bool objectsInAoe = true)
        {
            return
            (from u in SafeList(objectsInAoe)
                where u.Position.Distance2D(Player.Position) <= range &&
        ...
        {
            var clusterUnits =
            (SafeList(objectsInAoe).Where(u => u.ActorSnoId == 322236 &&
                                               u.Position.Distance2D(Player.Position) <= maxRange)
                .OrderBy(u => u.NearbyUnitsWithinDistance(radius))
                .ThenByDescending(u => u.Distance)
        ...
        public static Vector3 PointBehind(Vector3 point, float maxRange = 45, bool objectsInAoe = true)
        {
        ...
        public static int NearbyUnitsWithinDistance(TrinityActor actor, float range = 5f, bool unitsOnly = true)
        {
            ...
            return ObjectCache.Count(u => u.RActorId != actor.RActorId && u.IsUnit && u.Position.Distance2D(actor.Position) <= range && u.HasBeenInLoS);
        }

        public static IEnumerable<TrinityActor> NearbyTargets(TrinityActor actor, float range = 5f)
        {
            return ObjectCache.Where(u => u.RActorId != actor.RActorId && u.IsUnit && !u.IsSummonedByPlayer && u.Position.Distance(actor.Position) <= range && u.HasBeenInLoS);
        }
        ...
        internal static bool ClusterExists(float radius = 15f, float maxRange = 90f, int minCount = 2, bool forceElites = true)
        {
            ...
            if (forceElites && ObjectCache.Any(u => u.IsUnit && u.IsElite && u.Position.Distance2D(Player.Position) < maxRange))
                return true;
        ...
        internal static Vector3 GetClusterPoint(float clusterRadius = 15f, int minCount = 2)
        {
            ...
            if (ObjectCache.Any(u => u.IsUnit && u.IsElite && u.Position.Distance2D(Player.Position) < 200))
                return CurrentTarget.Position;

            var clusterUnit =
            (from u in ObjectCache
                where u.IsUnit && u.IsValid &&
                      u.Position.Distance2D(Player.Position) <= 200 &&
        ...
        //            where u.IsUnit &&
        //            u.Position.Distance2D(Player.Position) <= maxRange && u.HasBeenInLoS &&
        ...
        internal static TrinityActor GetBestArcTarget(float maxRange, float arcDegrees)
        {
            ...
            (from u in ObjectCache
                where u.IsUnit &&
                      u.Position.Distance2D(Player.Position) <= maxRange && !u.IsSafeSpot
                orderby u.CountUnitsInFront() descending
                select u).FirstOrDefault();
        ...
        internal static bool IsEliteTargetInRange(float range = 10f)
        {
            if (range < 5f)
                range = 5f;
            return TrinityCombat.Targeting.CurrentTarget != null && TrinityCombat.Targeting.CurrentTarget.IsElite && TrinityCombat.Targeting.CurrentTarget.Position.Distance2D(Player.Position) <= range;
        }
        ...
        internal static Vector3 GetZigZagTarget(Vector3 origin, float ringDistance = 20f, bool randomizeDistance = false)
        {
            ...
            if (origin.Distance2D(Player.Position) > ringDistance)
                return origin;
            ...
            if (useTargetBasedZigZag && ObjectCache.Count(o => o.IsUnit) >= minTargets)
            {
                ...
                {
                    Core.Logger.Log(LogCategory.Movement, "Returning ZigZag: BestClusterPoint {0} r-dist={1} t-dist={2}", clusterPoint, ringDistance, clusterPoint.Distance2D(Player.Position));
                    return clusterPoint;
                }
                ...
                if (attackInAoe)
                {
                    zigZagTargetList =
                    (from u in ObjectCache
                        where u.IsUnit && u.Position.Distance2D(Player.Position) < maxDistance && u.Weight > 0
                        select u).ToList();
                }
                else
                {
                    zigZagTargetList =
                    (from u in ObjectCache
                        where u.IsUnit && u.Position.Distance2D(Player.Position) < maxDistance && !UnitOrPathInAoE(u)
                        select u).ToList();
                }
                ...
                if (zigZagTargetList.Count() >= minTargets)
                {
                    ...
                    if (Grids.CanRayCast(zigZagPoint) && zigZagPoint.Distance2D(Player.Position) >= minDistance)
                    {
                        Core.Logger.Log(LogCategory.Movement, "Returning ZigZag: TargetBased {0} r-dist={1} t-dist={2}", zigZagPoint, ringDistance, zigZagPoint.Distance2D(Player.Position));
                        return zigZagPoint;
                    }
                }
            }
            ...
            foreach (float distance in distancePasses)
            {
                for (double direction = min; direction < max; direction += (Math.PI / piSlices))
                {
                    ...
                    // Get the Z
                    zigZagPoint.Z = DBGridProvider.GetHeight(zigZagPoint.ToVector2());
                    ...
                    if (pointWeight > highestWeightFound)
                    {
                        ...
                        //{
                        bestLocation = new Vector3(zigZagPoint.X, zigZagPoint.Y, DBGridProvider.GetHeight(zigZagPoint.ToVector2()));
                        //}
                        //else
                        //{
            ...
            Core.Logger.Log(LogCategory.Movement, "Returning ZigZag: RandomXY {0} r-dist={1} t-dist={2}", bestLocation, ringDistance, bestLocation.Distance2D(Player.Position));
            return bestLocation;
        }
        ...
        internal static bool IsPositionOnMonster(Vector3 position, float Distance = 0f)
        {
            if (position == Vector3.Zero)
                return false;

            return SafeList().Any(m => m.IsUnit && m.IsHostile && m.Position.Distance(position) <= m.CollisionRadius + Distance);
        }
        ...
        internal static bool IsPercentUnitsWithinBand(float bandMinRange = 10f, float bandMaxRange = 10f, double percentage = 50)
        {
            ...
            var totalWithinMaxRange = (from o in ObjectCache
                where o.IsUnit && o.Weight > 0 &&
                      o.Position.Distance2D(Player.Position) <= bandMaxRange
                select o).Count();

            var totalWithinMinRange = (from o in ObjectCache
                where o.IsUnit && o.Weight > 0 &&
                      o.Position.Distance2D(Player.Position) <= bandMinRange
                select o).Count();
        ...
        internal static float DebuffedPercent(SNOPower power, float maxRange, out List<TrinityActor> notDebuffed)
        {
            ...
            {
                var isValidUnit = u != null && u.IsUnit && u.Attributes != null && !u.IsPlayer && u.Position.Distance2D(Player.Position) <= maxRange && u.HasBeenInLoS;
                if (!isValidUnit)
                    continue;
        ...
        {
            return ObjectCache.Count(u => u.IsUnit && u.IsValid &&
                                           u.Position.Distance2D(Player.Position) <= maxRange &&
                                           powers.Any(u.HasDebuff));
        }

        internal static List<TrinityActor> UnitsWithoutDebuffs(List<SNOPower> powers, Vector3 location, float maxRange = 30f, IEnumerable<TrinityActor> units = null)
        {
            if (units == null)
                units = ObjectCache;

            return units.Where(u => u.IsUnit && u.IsValid &&
                                     u.Position.Distance(location) <= maxRange &&
                                     !powers.Any(u.HasDebuff) &&
                                     !powers.Any(p => SpellTracker.IsUnitTracked(u.AcdId, p))).ToList();
        }

        internal static IEnumerable<TrinityActor> UnitsWithOutDebuff(SNOPower power, Vector3 location,
            float maxRange = 30f, IEnumerable<TrinityActor> units = null)
        {
            if (units == null)
                units = ObjectCache;
            return units.Where(u => u.IsUnit && u.IsValid && u.Position.Distance(location) <= maxRange && !u.HasDebuff(power));
        }
        ...
        internal static TrinityActor BestTargetWithoutDebuff(float range, SNOPower debuff, Vector3 position)
        {
            TrinityActor target = ObjectCache.Where(u => u.IsUnit && u.IsValid &&
                                             u.Position.Distance2D(position) <= range &&
                                             !u.HasDebuff(debuff)).OrderByDescending(u => u.NearbyUnitsWithinDistance()).FirstOrDefault();
            return target;
        }
        ...
        internal static TrinityActor GetDashStrikeFarthestTarget(float maxRange, float procDistance = 33f, int arcDegrees = 0)
        {
            var result =
            (from u in ObjectCache
                where u.IsUnit && u.Position.Distance2D(Player.Position) >= procDistance && u.IsValid &&
                      u.Position.Distance2D(Player.Position) <= maxRange
                orderby u.Position.Distance2D(Player.Position) descending
                select u).FirstOrDefault();
            return result;
        }
            ...
            float procDistance = 33f, bool useWeights = true, bool includeUnitsInAoe = true)
        {
            ...
            (from u in ObjectCache
                where (u.IsUnit || (includeHealthGlobes && u.Type == TrinityObjectType.HealthGlobe)) &&
                      ((useWeights && u.Weight > 0) || !useWeights) &&
                      (includeUnitsInAoe || !UnitOrPathInAoE(u)) &&
                      u.Position.Distance2D(Player.Position) <= maxRange && u.Position.Distance2D(Player.Position) >= procDistance
        ...
        internal static TrinityActor GetClosestUnit(float maxDistance = 100f)
        {
            var result =
            (from u in ObjectCache
                where u.IsUnit && u.IsValid && u.Weight > 0 && u.Position.Distance2D(Player.Position) <= maxDistance
                orderby u.Distance
                select u).FirstOrDefault();
        ...
        {
            return (from u in ObjectCache
                where u.IsUnit && u.Position.Distance2D(Player.Position) <= maxRange && u.RiftValuePct >= minValuePercent
                orderby
        ...
        public static Vector3 GetLoiterPosition(TrinityActor target, float Distance)
        {
            if (target == null)
                return Vector3.Zero;

            var circlePositions =
                StuckHandler.GetCirclePoints(16, target.AxialRadius + Distance, target.Position);
            circlePositions.AddRange(StuckHandler.GetCirclePoints(16, target.AxialRadius + Distance + 15f,
                target.Position));

            Func<Vector3, bool> isValid = p => Grids.CanRayCast(p) && !IsPositionOnMonster(p, Distance);
            ...
            validPositions = validPositions.OrderBy(x => UnitsInRangeOfPosition(x))
                .ThenBy(p => p.Distance2D(Player.Position)).ToList();
        ...
        public static Vector3 GetSafeSpotPosition(float distance)
        {
            ...
            var safeSpotIsClose = _lastSafeSpotPosition.Distance2D(Player.Position) < distance;
            ...
            Func<Vector3, bool> isValid = p => Grids.CanRayCast(p) && !IsPositionOnMonster(p);
            ...
            out double clusterValue)
        {
            var units = (from u in ObjectCache
                where u.IsUnit && u.Position.Distance2D(Player.Position) <= maxRange &&
                      u.RiftValuePct > 0 &&
        ...
        public static IEnumerable<TrinityActor> Players => Actors.Actors.Where(x => x.IsPlayer);

        public static bool AnyPlayer(Func<TrinityActor, bool> condition)
        {
            return Players.Any(condition);
        }

        public static TrinityActor IsEliteInRange(float range, bool objectsInAoe = true)
        {
        ...
        public static TrinityActor BestLOSEliteInRange(float range, bool objectsInAoe = true)
        {
            return (from u in SafeList(objectsInAoe)
                where u.IsUnit &&
                      (u.IsElite || u.IsRare || u.IsChampion || u.IsBoss) &&
                      u.IsInLineOfSight &&
                      u.Position.Distance2D(Player.Position) <= range
                orderby
                u.NearbyUnitsWithinDistance(15) descending,
        ...
        public static TrinityActor BestRangedAoeUnit(float clusterRadius = 7, float maxSearchRange = 50, int unitCount = 3, bool useWeights = false, bool includeUnitsInAoE = true)
        {
            return BestLOSEliteInRange(maxSearchRange, includeUnitsInAoE) ??
                   GetFarthestClusterUnit(Player.Position, clusterRadius, maxSearchRange, unitCount, useWeights, includeUnitsInAoE);
        }
        ...
        public static Vector3 GetBestCorpsePoint(float rangeFromPlayer, float targetRegionRadius)
        {
            ...
            var clusterCorpse = (from u in corpses
                ...
                u.Position.Distance2D(Player.Position) ascending
                select u).FirstOrDefault();
        ...
        public static int CorpseCountNearLocation(Vector3 location, float radius)
            => Actors.Count(a => GameData.NecroCorpseSnoIds.Contains(a.ActorSnoId) && a.Position.Distance(location) <= radius && a.IsInLineOfSight);

        public static int CorpseCount(float radius)
            => Actors.Count(a => GameData.NecroCorpseSnoIds.Contains(a.ActorSnoId) && a.Position.Distance2D(Player.Position) <= radius && a.IsInLineOfSight);

        public static int CorpseCount(float radius, Vector3 position)
            => Actors.Count(a => GameData.NecroCorpseSnoIds.Contains(a.ActorSnoId) && a.Position.Distance(position) <= radius && a.IsInLineOfSight);

        public static IEnumerable<TrinityActor> GetCorpses(float radius)
            => Actors.Where(a => GameData.NecroCorpseSnoIds.Contains(a.ActorSnoId) && a.Position.Distance2D(Player.Position) <= radius && a.IsInLineOfSight);

        public static IEnumerable<TrinityActor> GetCorpses(float radius, Vector3 position)
            => Actors.Where(a => GameData.NecroCorpseSnoIds.Contains(a.ActorSnoId) && a.Position.Distance(position) <= radius && a.IsInLineOfSight);
