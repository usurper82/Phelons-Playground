...
{
    public sealed class WitchDoctorLoNBarrage : WitchDoctorBase, IRoutine
    {
        #region Definition

        public string DisplayName => "LoN Barrage";
        public string Description => "Specialized combat for channelling Barrage with LoN effect.";
        public string Author => "xzjv";
        public string Version => "0.1";
		public string Url => "https://www.icy-veins.com/d3/witch-doctor-legacy-of-nightmares-spirit-barrage-build-patch-2-6-season-11";
        ...
        {
            var isSpiritBarrageSelected = TrinityCombat.Targeting.CurrentPower?.SNOPower == SNOPower.Witchdoctor_SpiritBarrage;
            var isInAvoidance = Core.Avoidance.InCriticalAvoidance(Player.Position);
            if (isSpiritBarrageSelected && TargetUtil.AnyMobsInRange(20f) && Player.CurrentHealthPct > 0.5f && !isInAvoidance)
                return true;
        ...
        public TrinityPower GetOffensivePower()
        {
            ...
            if (bestClusterUnit != null)
            {
                ...
                var interruptForHarvest = Skills.WitchDoctor.SoulHarvest.CanCast() && harvestPossibleStackGain >= harvestUnitsInRange && harvestBuffCooldown?.Remaining.TotalMilliseconds < 5000;
                ...
                if (Player.IsChannelling)
                {
                    ...
                    {
                        Core.Logger.Log(LogCategory.Routine, "Continuation of SpiritBarrage.");
                        return new TrinityPower(SNOPower.Witchdoctor_SpiritBarrage, 40f, Player.Position, 75, 250);
                    }

                    if (interruptForHaunt)
                        Core.Logger.Log(LogCategory.Routine, "Interrupted SpiritBarrage to haunt");

                    if (interruptForLocust)
                        Core.Logger.Log(LogCategory.Routine, "Interrupted SpiritBarrage to locust");

                    if (interruptForHarvest)
                        Core.Logger.Log(LogCategory.Routine, "Interrupted SpiritBarrage to harvest");
                }
                ...
                if (distance > 10f && !PlayerMover.IsBlocked)
                {
                    ...
                    //return new TrinityPower(SNOPower.Walk, 20f, bestSpiritBarragePosition, 20, 20);
                }
                ...
                if (Skills.WitchDoctor.SpiritBarrage.CanCast())
                {
                    ...
                    if (closestUnit != null)
                    {
                        Core.Logger.Log(LogCategory.Routine, $"Casting Spirit Barrage");
        ...
        public WitchDoctorLoNBarrageSettings Settings { get; } = new WitchDoctorLoNBarrageSettings();

        public sealed class WitchDoctorLoNBarrageSettings : NotifyBase, IDynamicSetting
        {
